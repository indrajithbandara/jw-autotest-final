...
#rdbms db postgres connection
conn = psycopg2.connect(database=jwdb, user=jwuser, password=jwpasswd, host=jwhost, port=jwport)
#conn = psycopg2.connect(database="jw_platform", user="platform", password="123456", host="192.168.18.206", port="5432")
cur = conn.cursor()

#rdbms db mysql connection
...
def getIFD_INTERFACEDEF_Count():
    """
       query record---> query all record from IFD_INTERFACEDEF table
    """
    select_sql = "select count(*) from IFD_INTERFACEDEF"
    print "the getIFD_INTERFACEDEF_All sql statement is ->",select_sql
    cur.execute(select_sql)
    result = cur.fetchall()
    if result == None: return "N"
...
def getIFD_EXAMPLEDEF_From_IFD_INTERFACEDEF(INTER_NAME):
    ...
    select_sql = "SELECT * FROM IFD_EXAMPLEDEF where INFNM=%s"
...
def getIFD_EXPINPUT_From_EXAMPLEDEF_ID(EXAMPLEDEF_ID):
    ...
    select_sql = "SELECT * FROM IFD_EXPINPUT where EXAMPLEDEF_ID=%s"
    if EXAMPLEDEF_ID == "N": return "N"
    print "the getIFD_EXPINPUT_From_EXAMPLEDEF_ID sql statement is ->",select_sql,EXAMPLEDEF_ID
...
def getIFD_EXPOUTPUT_From_EXAMPLEDEF_ID(EXAMPLEDEF_ID):
    ...
    """
    print " the pars EXAMPLEDEF_ID is ===========>",EXAMPLEDEF_ID
    select_sql = "SELECT * FROM IFD_EXPOUTPUT where EXAMPLEDEF_ID=%s"
    if EXAMPLEDEF_ID == "N": return "N"
...
def getIFD_EXPASSERT_From_EXAMPLEDEF_ID(EXAMPLEDEF_ID):
    ...
    """
    print " the pars EXAMPLEDEF_ID is ===========>",EXAMPLEDEF_ID
    select_sql = "SELECT * FROM IFD_EXPASSERT where EXAMPLEDEF_ID=%s"
    if EXAMPLEDEF_ID == "N": return "N"
...
def main():
     ...
     print " the selectIfd_interfacedef is >>>>>>>>>>>>>>>>>>>>>",selectIfd_interfacedef()
     print " the selectIND_DTOFIELD is >>>>>>>>>>>>>>>>>>>>>",selectIND_DTOFIELD()
     # getIFD_INTERFACEDEF_All()
     result=getIFD_INTERFACEDEF_All()
     print "=========== intefface define api result is ->",result
     for inter_row in result:
        print "===========inter_row row[3]->",inter_row[3]
        result_case=getIFD_EXAMPLEDEF_From_IFD_INTERFACEDEF("infnm1")
